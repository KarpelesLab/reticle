...
//!
//! **The library does not fetch it.** Every byte comes through a
//! [`FileProvider`] the caller hands in, and the caller says where the
//! file is; the command line downloads a pinned, hash-checked copy (`reticle
//! fetch apicula`). The database is not in this repository and must not
//! be; without it every test here skips and says so.
//!
//! # One design from this loader has run on a part
//!
//! On 2026-09-24 a button through one lookup table to an LED, placed and
//! routed over this loader's fabric and configured by `config`, ran on
//! a Sipeed Tang Primer 20K (a GW2A-LV18PG256C8/I7), loaded by `reticle
//! program`, and a person watched the LED follow the button. Nothing
//! clocked has been tried; the reasons it would not yet route are below.
//!
//! What stands behind that, and against what, is in `docs/fpga-gowin.md`.
//! The short form: for three designs `gowin_pack`, Apicula's own packer,
//! writes from the same placement and routing — nothing, two IO buffers,
//! and a routed LUT between them — [`ApiculaFabric::stream`] writes the
//! same `.fs` byte for byte, and the fabric's measured size comes out of
//! [`ApiculaStats`] rather than out of a note.
//!
//! # The database
//!
...
//!
//! # What the database does not name, and where it comes from instead
//!
...
//! Python dictionaries in `apycula/attrids.py`; the four this flow needs
//! are transcribed in `attrids`, generated by
//! `tools/gen-apicula-attrids.py`. With them, `config` configures what
//! `gowin_pack` configures for an IO buffer, an unused IO, a bank, and a
//! slice with empty registers. All of it is [`Periphery`], per bank, and
//! [`ApiculaFabric::stream`] applies it from what the design occupies: an
//! IO buffer's bits are not bel entries, because a bel's entries belong to
//! its tile *type* and one type of IO tile sits in several banks.
//!
...
pub use config::{Bank, IO_STANDARDS, IoSite, Periphery, PeripheryError, TileBits, bank_vccio};
...
pub enum ApiculaError {
    ...
    /// A design this flow cannot configure.
    Periphery(PeripheryError),
    /// An IO standard this flow does not configure.
    UnsupportedIoStandard {
        /// The standard asked for.
        standard: String,
        /// The ones it does.
        known: Vec<String>,
    },
...
impl fmt::Display for ApiculaError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            ApiculaError::Periphery(err) => write!(f, "{err}"),
            ApiculaError::UnsupportedIoStandard { standard, known } => write!(
                f,
                "IO standard `{standard}` is not one this flow configures; it configures {}",
                known.join(", ")
            ),
        }
    }
...
pub struct ApiculaOptions {
    ...
    pub max_pips: usize,
    /// The IO standard a bank [`bank_io_standards`](Self::bank_io_standards)
    /// does not name is configured for, `LVCMOS33` by default. See
    /// [`Periphery`] and [`IO_STANDARDS`].
    pub io_standard: String,
    /// The IO standard of each bank that has one of its own, by bank
    /// number. Every used IO in a bank is configured for that bank's
    /// standard, which has to be what the board supplies the bank with.
    pub bank_io_standards: BTreeMap<i64, String>,
}
...
impl Default for ApiculaOptions {
    fn default() -> Self {
        ApiculaOptions {
            ...
            bank_io_standards: BTreeMap::new(),
        }
    }
}
...
impl ApiculaOptions {
    ...
    /// The same options with every bank not named otherwise configured
    /// for `standard`.
    pub fn with_io_standard(mut self, standard: impl Into<String>) -> ApiculaOptions {
        self.io_standard = standard.into();
        self
    }

    /// The same options with bank `bank` configured for `standard`.
    pub fn with_bank_io_standard(
        mut self,
        bank: i64,
        standard: impl Into<String>,
    ) -> ApiculaOptions {
        self.bank_io_standards.insert(bank, standard.into());
        self
    }
}
...
pub struct ApiculaStats {
    ...
    /// bits: an IO buffer's depend on its bank, so they are
    /// [`Periphery`]'s.
...
pub struct ApiculaFabric {
    ...
    /// The configuration no routed net carries: IO buffers, used and
    /// unused, the banks, and the defaults of a slice with empty
    /// registers.
...
impl ApiculaFabric {
    ...
    pub fn blank_stream(&self) -> Result<FsStream, ApiculaError> {
        self.stream(&[], &BTreeMap::new())
    }

    /// A whole `.fs` stream for a design: the `const` bits, `design` —
    /// each tile's own bits, the tile as `(x, y)`, which is what
    /// [`bitstream::generate`](crate::fpga::bitstream::generate) produces —
    /// and the [`Periphery`] for a design occupying `used`, each `(tile,
    /// bel)` its cells sit on with the primitive placed there. The
    /// footer's checksum is filled in.
    ///
    /// With nothing placed this is what `gowin_pack` writes for a design
    /// with nothing in it, bit for bit; `tests/fpga_gowin.rs` checks it.
    ///
    /// # Errors
    ///
    /// [`ApiculaError::Container`] when the header has no
    /// load-configuration command to write the row count into, or the
    /// footer no checksum line; [`ApiculaError::Periphery`] for an IO
    /// primitive the flow does not configure.
    pub fn stream(
        &self,
        design: &[((u32, u32), Vec<ConfigBit>)],
        used: &BTreeMap<((u32, u32), String), String>,
    ) -> Result<FsStream, ApiculaError> {
        let mut bitmap = self.layout.bitmap();
        let mut set = |(x, y): (u32, u32), bits: &[ConfigBit]| {
            if let Some((origin_row, origin_col)) = self.layout.origin(y, x) {
                for bit in bits {
        ...
        for (tile, bits) in design {
            set(*tile, bits);
        }
        for (tile, bits) in self.periphery.bits(used).map_err(ApiculaError::Periphery)? {
            set(tile, &bits);
        }
        let mut stream = FsStream::new(self.header.clone(), bitmap, self.footer.clone())?;
        stream.fill_checksum()?;
        Ok(stream)
    }
}
...
impl ApiculaDatabase {
    ...
    pub fn load(&self, options: &ApiculaOptions) -> Result<ApiculaFabric, ApiculaError> {
        ...
        // Refuse an IO standard before building anything for it.
        let periphery = self.periphery(&options.io_standard, &options.bank_io_standards)?;
