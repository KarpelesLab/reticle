...
//! slice with empty registers. An IO bel carries its `IBUF` and `OBUF`
//! bits as [`ConfigEntry::Cell`] entries for the load's
//! [`ApiculaOptions::io_standard`]; the rest is [`Periphery`], applied by
//! [`ApiculaFabric::stream`] from what the design occupies.
//!
...
pub use config::{Bank, IO_STANDARDS, IdleIo, Periphery, bank_vccio};
...
pub struct ApiculaOptions {
    ...
    pub max_pips: usize,
    /// The IO standard every buffer is configured for, `LVCMOS33` by
    /// default. It is one per load because the bits are worked out per
    /// load; see [`Periphery`] and [`IO_STANDARDS`].
...
impl ApiculaOptions {
    ...
    /// The same options with every buffer configured for `standard`.
...
pub struct ApiculaStats {
    ...
    /// Configuration entries the loader emitted: lookup-table bits, and an
    /// `IBUF` and an `OBUF` entry per IO bel.
...
pub struct ApiculaFabric {
    ...
    /// The configuration no placed cell carries: unused IO, the banks,
    /// and the defaults of a slice with empty registers.
...
impl ApiculaFabric {
    ...
    /// and the [`Periphery`] for a design occupying `used`, the `(tile,
    /// bel)` pairs its cells sit on. The footer's checksum is filled in.
    ///
    ...
    /// footer no checksum line.
    pub fn stream(
        &self,
        design: &[((u32, u32), Vec<ConfigBit>)],
        used: &BTreeSet<((u32, u32), String)>,
        ...
        for (tile, bits) in self.periphery.bits(used) {
            set(tile, &bits);
        }
...
impl ApiculaDatabase {
    ...
    pub fn load(&self, options: &ApiculaOptions) -> Result<ApiculaFabric, ApiculaError> {
        ...
        let periphery = self.periphery(&options.io_standard)?;
        let iob_codes = config::Codes::iob(self);
        ...
        for ttyp in &used {
            ...
            self.configure_io_bels(&iob_codes, *ttyp, &mut tile_type, &options.io_standard)?;
